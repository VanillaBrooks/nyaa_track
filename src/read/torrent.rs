...
impl ToBencode for File {
    fn to_bencode(&self) -> Bencode {
        ...
        if self.ed2k.is_some() {
            m.insert(
                ByteString::from_str("ed2k"),
                Bencode::ByteString(self.ed2k.clone().unwrap().to_vec()),
            );
        }

        if self.filehash.is_some() {
            m.insert(
                ByteString::from_str("filehash"),
                Bencode::ByteString(self.filehash.clone().unwrap().to_vec()),
            );
        }

        if self.attr.is_some() {
            m.insert(
                ByteString::from_str("attr"),
                self.attr.clone().unwrap().to_bencode(),
            );
        }
        ...
        if self.utf8path.is_some() {
            m.insert(
                ByteString::from_str("path.utf-8"),
                self.utf8path.clone().unwrap().to_bencode(),
            );
        }

        if self.sha1.is_some() {
            m.insert(
                ByteString::from_str("sha1"),
                Bencode::ByteString(self.sha1.clone().unwrap().to_vec()),
            );
        }

        if self.md5sum.is_some() {
            m.insert(
                ByteString::from_str("md5sum"),
                self.md5sum.clone().unwrap().to_bencode(),
            );
        }
...
impl ToBencode for Info {
    fn to_bencode(&self) -> Bencode {
        ...
        if self.fileduration.is_some() {
            m.insert(
                ByteString::from_str("file-duration"),
                self.fileduration.clone().unwrap().to_bencode(),
            );
        }

        if self.filemedia.is_some() {
            m.insert(
                ByteString::from_str("file-media"),
                self.filemedia.clone().unwrap().to_bencode(),
            );
        }
        ...
        if self.ed2k.is_some() {
            m.insert(
                ByteString::from_str("ed2k"),
                Bencode::ByteString(self.ed2k.clone().unwrap().into_vec()),
            );
        }
        ...
        if self.filehash.is_some() {
            m.insert(
                ByteString::from_str("filehash"),
                Bencode::ByteString(self.filehash.clone().unwrap().into_vec()),
            );
        }
        ...
        if self.piece_length.is_some() {
            m.insert(
                ...
                self.piece_length.unwrap().to_bencode(),
            );
        }
        ...
        if self.pieces.is_some() {
            m.insert(
                ...
                Bencode::ByteString(self.pieces.clone().unwrap().into_vec()),
            );
        }
        ...
        if self.private.is_some() {
            m.insert(
                ...
                self.private.unwrap().to_bencode(),
            );
        }

        if self.source.is_some() {
            m.insert(
                ByteString::from_str("source"),
                self.source.clone().unwrap().to_bencode(),
            );
        }
